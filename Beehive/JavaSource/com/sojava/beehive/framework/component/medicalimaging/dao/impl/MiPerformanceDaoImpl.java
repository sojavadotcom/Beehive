...
class SharingMode implements Mode {
	...
	public void calMainInfo() throws Exception {
		...
		// TODO 待重新处理以下两个查询，从VMiExecuted查询方式，目前存在大数据引起的性能下降
...
*/
			stmt = this.querySession.createSQLQuery(
					...
					+ "price,"
					+ "point,"
					+ "count(rbrvs_id) as quantity,"
					+ "coalesce(worker1_id, 0) as worker1Id,"
					+ "coalesce(worker1_name, '') as worker1Name,"
					+ "coalesce(worker1_coef, 0) as worker1Coef,"
					+ "coalesce(worker2_id, 0) as worker2Id,"
					+ "coalesce(worker2_name, '') as worker2Name,"
					+ "coalesce(worker2_coef, 0) as worker2Coef,"
					+ "type,"
					+ "kind,"
					+ "dept,"
					+ "coalesce(execute_diagnostician_is_student, 0) as executeDiagnosticianIsStudent"
					+ " from "
					+ "("
					+ "	select "
					...
					+ " 	and a.report_date between :begin and :end"
					+ " 	and b.work_statistics_id=:workStatisticsId"
					+ "		union all "
					+ "		select "
					+ "		b.work_statistics_id,"
					+ "		b.id as rbrvs_price_id,"
...
//					+ " and work_statistics_id=:workStatisticsId"
					+ " group by rbrvs_price_id,rbrvs_id,price,point,worker1_id,worker1_name,worker1_coef,worker2_id,worker2_name,worker2_coef,type,kind,dept,execute_diagnostician_is_student"
				);
